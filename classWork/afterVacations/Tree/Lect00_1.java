...
public class Lect00_1 {
    ...
    public static int pathSum(Node root, int targetSum) {
        ...
        int[] ans = { 0 };
        recursiveSol(root, ans, targetSum);

        return ans[0];
    }

    public static int[] recursiveSol(Node node, int[] ans, int target) {
        ...
        if (node.left != null) {
            ...
            recLeft = recursiveSol(node.left, ans, target);
        ...
        if (node.right != null) {
            ...
            recRight = recursiveSol(node.right, ans, target);

        }

        if (recLeft != null && recLeft[0] != Integer.MIN_VALUE && recLeft[0] + node.data == target) {
            ans[0]++;
        }

        if (recLeft != null && recLeft[1] != Integer.MIN_VALUE && recLeft[1] + node.data == target) {
            ans[0]++;
        }

        if (recRight != null && recRight[0] != Integer.MIN_VALUE && recRight[0] + node.data == target) {
            ans[0]++;
        }

        if (recRight != null && recRight[1] != Integer.MIN_VALUE && recRight[1] + node.data == target) {
            ans[0]++;
        }
        ...
        if (node.left != null) {
            ...
            if (recLeft[0] == Integer.MIN_VALUE) {
                  
            } else {
                lsum = recLeft[0] + node.data;
            }
        }

        if (node.right != null) {
            rsum = recRight[1] + node.data;
        }
